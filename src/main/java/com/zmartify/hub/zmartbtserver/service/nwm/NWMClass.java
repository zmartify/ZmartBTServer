...
public class NWMClass {
	...
	*/
	enum NM80211ApFlags {
		...
		NM_802_11_AP_FLAGS_PRIVACY(0x00000001,"access point requires authentication and encryption (usually means WEP)");
	...
	*/
	enum NM80211Mode {
		...
		NM_802_11_MODE_ADHOC(1,"for both devices and access point objects, indicates the object is part of an Ad-Hoc 802.11 network without a central coordinating access point."),
		NM_802_11_MODE_INFRA(2,"the device or access point is in infrastructure mode. For devices, this indicates the device is an 802.11 client/station. For access point objects, this indicates the object is an access point that provides connectivity to clients."),
		NM_802_11_MODE_AP(3,"the device is an access point/hotspot. Not valid for access point objects; used only for hotspot mode on the local machine.");
	...
	*/
	enum NMDeviceModemCapabilities {
		...
		NM_DEVICE_MODEM_CAPABILITY_POTS(0x00000001,"modem uses the analog wired telephone network and is not a wireless/cellular device"),
		NM_DEVICE_MODEM_CAPABILITY_CDMA_EVDO(0x00000002,"modem supports at least one of CDMA 1xRTT, EVDO revision 0, EVDO revision A, or EVDO revision B"),
		NM_DEVICE_MODEM_CAPABILITY_GSM_UMTS(0x00000004,"modem supports at least one of GSM, GPRS, EDGE, UMTS, HSDPA, HSUPA, or HSPA+ packet switched data capability"),
	...
	*/
	enum NMDeviceStateReason {
		...
		NM_DEVICE_STATE_REASON_IP_CONFIG_UNAVAILABLE(5,"IP configuration could not be reserved (no available address, timeout, etc)"),
	...
	*/
	enum NMSecretAgentGetSecretsFlags {
		NM_SECRET_AGENT_GET_SECRETS_FLAG_NONE(0x0,"no special behavior; by default no user interaction is allowed and requests for secrets are fulfilled from persistent storage, or if no secrets are available an error is returned."),
		NM_SECRET_AGENT_GET_SECRETS_FLAG_ALLOW_INTERACTION(0x1,"allows the request to interact with the user, possibly prompting via UI for secrets if any are required, or if none are found in persistent storage."),
		NM_SECRET_AGENT_GET_SECRETS_FLAG_REQUEST_NEW(0x2,"explicitly prompt for new secrets from the user. This flag signals that NetworkManager thinks any existing secrets are invalid or wrong. This flag implies that interaction is allowed."),
		NM_SECRET_AGENT_GET_SECRETS_FLAG_USER_REQUESTED(0x4,"set if the request was initiated by user-requested action via the D-Bus interface, as opposed to automatically initiated by NetworkManager in response to (for example) scan results or carrier changes."),
	...
	*/
	enum NMSecretAgentCapabilities {
		...
		NM_SECRET_AGENT_CAPABILITY_VPN_HINTS(0x1,"the agent supports passing hints to VPN plugin authentication dialogs.");
